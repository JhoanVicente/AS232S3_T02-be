-- * Proceso Almacenado para gestión del Administrador
...
BEGIN
    ...
    FROM
        product.detail_product dp
            JOIN product.product p ON dp.id_product = p.id_product
    WHERE
...
-- * Proceso Almacenado para gestión del Cliente
...
BEGIN
    SELECT
        ...
        CASE
            ...
            ELSE 'Entrega en el Restaurante'
            END AS 'Método de entrega',
        -- Datos del Tipo de Pago tp
        tp.name AS 'Tipo de pago',
        -- Datos del Tipo de Estado de Pedido tso
        tso.name AS 'Estado',
        -- Datos del usuario u
        u.names AS 'Nombres',
        u.lastnames AS 'Apellidos',
...
-- * Todos los pedidos por paginación
